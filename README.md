# PaveVision PG-PDN Demonstration Package

[![CI](https://github.com/GanghaoSun/pavevision-pgpdn-demo/actions/workflows/ci.yml/badge.svg)](https://github.com/GanghaoSun/pavevision-pgpdn-demo/actions/workflows/ci.yml)

This repository accompanies the manuscript "Physics-guided pavement degradation prediction from grid-level semantic distress maps".

It is the public software and demonstration package for inspecting the PG-PDN model structure, the PaveVision visualization workflow and the processed route-level outputs used by the browser demo. The repository is organized so that readers can run the web system, inspect the feature schema, execute the model architecture on a synthetic table, and review the manuscript-aligned visualizations without external project files.

## Repository Contents

- `web_demo/`: Flask-based PaveVision web system serving processed pavement quality JSON files and precomputed PG-PDN route outputs.
- `pgpdn/`: lightweight Python implementation of the PG-PDN architecture, feature validation utilities and plotting helpers.
- `configs/pgpdn_default.yaml`: architecture, physical-branch and loss defaults used by the public package.
- `examples/`: runnable scripts for schema validation, model-interface inspection and synthetic map generation.
- `sample_data/`: a small synthetic feature table for checking input and output formats.
- `web_demo/data/sample/`: processed pavement quality JSON files for three routes, three survey periods and the 0.5 m visualization workflow.
- `assets/`: PaveVision screenshots, PG-PDN architecture illustration and manuscript-style grid-level visualization examples.
- `supplementary/`: method notes and release-scope documentation for the public package.
- `docs/`: concise guides for the public workflow, data schema and manuscript-to-code mapping.
- `tests/`: local checks for the model interface, web API, manifest consistency and public-facing text.

## Update Record

| Date | Uploaded or updated content | Notes |
| --- | --- | --- |
| 2026-06-28 | Refined the public release narrative, added documentation pages, CI tests, citation metadata and changelog, and aligned web-demo model descriptions with the included precomputed outputs. | Public pages now emphasize the materials that are available, runnable and inspectable in this repository. |
| 2026-06-07 | Updated the PaveVision web system to the full processed pavement quality data for all three routes (about 5.40 km in total), real PG-PDN prediction outputs and per-route baseline comparison, refreshed interface screenshots, continuous-gradient grid heatmaps and a responsive maintenance-unit view. | Displayed numbers are aligned with the manuscript results. |
| 2026-06-05 | Updated repository release statement and added this update record. | Clarified the public package structure and inspection workflow. |
| 2026-06-04 | Added PaveVision interface screenshots, PG-PDN architecture illustration and paper-aligned visualization examples. | Supports visual inspection of the route map, quality assessment, performance prediction and grid-level prediction outputs. |
| 2026-06-03 | Added the public PaveVision web demo, PG-PDN inference package, configuration template, example scripts and demonstration data files. | Provides runnable materials for method inspection and software reuse. |

## PaveVision Interface Preview

PaveVision is a pavement quality assessment and performance-prediction interface built around semantic distress maps and grid-level PG-PDN outputs. The screenshots below show the route map, pavement quality assessment and one-step-ahead prediction views served by the public web demo.

### Route Map and Quality Popup

The route-map view provides the first entry point for pavement engineers. Clicking a route opens a quality summary popup with the survey period, route length, mean PQI*, distress deduction, number of 20 m maintenance units and 0.5 m grid resolution.

![PaveVision route map with pavement quality popup](assets/web_route_map_popup.png)

### Pavement Quality Assessment

![PaveVision pavement quality assessment module](assets/web_quality_module.png)

### Performance Prediction

![PaveVision performance prediction module](assets/web_prediction_module.png)

### PG-PDN Network Architecture

The PG-PDN architecture combines an interpretable physical degradation branch with a GRU residual correction branch. This package keeps the model interface, feature schema, loss terms and visualization workflow consistent with the manuscript.

![PG-PDN network architecture](assets/pgpdn_architecture.png)

### Historical Measurements and PG-PDN Prediction at 0.5 m Resolution

The fig10 series visualizes Route 3 at 0.5 m grid resolution: March 2024 measured quality, March 2025 measured quality, March 2026 PG-PDN prediction, March 2026 measured quality and the prediction residual. These images illustrate measured-to-predicted spatial continuity rather than a comparison among different learning algorithms.

#### March 2024 Measured

![Route 3 March 2024 measured grid-level PQI*](assets/fig10a_route3_2024_measured.png)

#### March 2025 Measured

![Route 3 March 2025 measured grid-level PQI*](assets/fig10b_route3_2025_measured.png)

#### March 2026 Predicted

![Route 3 March 2026 PG-PDN predicted grid-level PQI*](assets/fig10c_route3_2026_predicted.png)

#### March 2026 Measured

![Route 3 March 2026 measured grid-level PQI*](assets/fig10d_route3_2026_measured.png)

#### Prediction Residual

![Route 3 grid-level PG-PDN prediction residual](assets/fig10e_route3_prediction_residual.png)

## Quick Start

Run the public PaveVision web demo:

```bash
python -m pip install -r web_demo/requirements.txt
python web_demo/app.py
```

On Windows, the one-click launcher can be used from the repository root:

```bat
run_web_demo.bat
```

Then open:

```text
http://localhost:5000
```

The web system's prediction view loads the included precomputed PG-PDN outputs from `web_demo/data/sample/`.

For the Python PG-PDN architecture examples, install the minimal modeling dependencies:

```bash
pip install -r requirements.txt
```

Run the synthetic inference example:

```bash
python examples/run_inference_template.py --features sample_data/synthetic_grid_features.csv
```

Generate a synthetic quality-map preview:

```bash
python examples/plot_synthetic_quality_map.py --features sample_data/synthetic_grid_features.csv --output outputs/synthetic_quality_map.png
```

The default model uses initialized physical parameters and randomly initialized neural parameters. The numerical outputs from the synthetic example are intended for interface verification.

## Feature Schema

Each grid cell is represented by the 12-dimensional feature vector used in the manuscript:

```text
[PQI*, ESAL, P, DeltaT, F, D1, D2, D3, D4, I_mean, I_std, I_low]
```

where `PQI*` is the current grid-level pavement quality index, `ESAL` is expressed in units of 10^4, `P` is precipitation in mm, `DeltaT` is the mean daily temperature range in degrees Celsius, `F` is the number of low-temperature days with mean daily temperature below 0 degC, `D1`--`D4` are distress densities for transverse cracks, longitudinal cracks, alligator cracks and potholes, and `I_mean`, `I_std`, `I_low` are LiDAR intensity features.

See [docs/data_schema.md](docs/data_schema.md) for the public JSON and CSV schema details.

## Public Inspection Workflow

1. Run `python -m unittest discover -s tests` to check package imports, model outputs, web API endpoints and data manifest consistency.
2. Start the PaveVision web demo and inspect route-level assessment and prediction views.
3. Review `pgpdn/model.py`, `pgpdn/features.py` and `configs/pgpdn_default.yaml` for the PG-PDN model interface.
4. Use `examples/run_inference_template.py` with `sample_data/synthetic_grid_features.csv` to verify the feature schema and model data flow.
5. Review [docs/manuscript_mapping.md](docs/manuscript_mapping.md) to connect manuscript concepts with repository files.

## Citation

Please cite the associated manuscript if you use this code or adapt the PG-PDN architecture. Citation metadata are provided in [CITATION.cff](CITATION.cff).
